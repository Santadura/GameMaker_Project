...
if (!instance_exists(obj_player1_2)) {  // Thay obj_player3 = tên player của bạn
    exit;
}

var dist = distance_to_object(obj_player1_2);

// Xác định hướng player
if (obj_player1_2.x < x) {
...
if (state == "chase") {
    ...
    if (dist < attack_range && attack_cooldown <= 0) {
        ...
        move_towards_point(obj_player1_2.x, obj_player1_2.y, chase_speed);
...
function trigger_quiz() {
    ...
    if (!obj_quiz_manager.show_quiz) {
        ...
        obj_quiz_manager.set_subject_and_level("biology", 2);
