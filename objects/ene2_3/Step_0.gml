...
if (!instance_exists(obj_player2_3)) {  // Thay obj_player3 = tên player của bạn
    exit;
}

var dist = distance_to_object(obj_player2_3);

// Xác định hướng player
if (obj_player2_3.x < x) {
...
if (state == "chase") {
    ...
    if (dist < attack_range && attack_cooldown <= 0) {
        ...
        move_towards_point(obj_player2_3.x, obj_player2_3.y, chase_speed);
...
function trigger_quiz() {
    ...
    if (!obj_quiz_manager.show_quiz) {
        ...
        obj_quiz_manager.set_subject_and_level("physics", 3);
